...
let () =
  let file1 = "data/csv/input1.csv" in
  let file2 = "data/csv/input2.csv" in
  let joined_file = "data/csv/joined.csv" in
  let output_file = "data/csv/output.csv" in
  let normalized_output_file = "data/csv/normalized_output.csv" in
  ...
  if not (Sys.file_exists file1) || not (Sys.file_exists file2) then
    ...
    let schema1, stream1 = Csv_reader.read_csv_with_schema file1 in
    let schema2, stream2 = Csv_reader.read_csv_with_schema file2 in
    ...
    Csv_writer.write_csv joined_file joined_schema joined_stream;
    ...
    let _, agg_stream = Csv_reader.read_csv_with_schema joined_file in
    ...
    let schema_joined, stream_joined = Csv_reader.read_csv_with_schema joined_file in
    ...
    Csv_writer.write_csv output_file schema_joined processed_stream;
    ...
    let schema_out, stream_ns_stats = Csv_reader.read_csv_with_schema output_file in
    ...
    let _, stream_ns_apply = Csv_reader.read_csv_with_schema output_file in
    let normalized_stream = Float_ops.standardize "Salary" mean_sal std_dev_sal stream_ns_apply in
    Csv_writer.write_csv normalized_output_file schema_out normalized_stream;
    ...
    let _, stream_for_age = Csv_reader.read_csv_with_schema output_file in
    ...
    let _, stream_for_salary = Csv_reader.read_csv_with_schema normalized_output_file in
