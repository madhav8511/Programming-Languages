...
let () =
  ...
  if not (Sys.file_exists file1) || not (Sys.file_exists file2) then
    ...
    let _, agg_stream = Json_reader.read_json_with_schema joined_file in
    ...
    let schema_out, stream_ns_stats = Json_reader.read_json_with_schema output_file in
    ...
    let _, stream_for_age = Json_reader.read_json_with_schema output_file in
    ...
    let _, stream_for_salary = Json_reader.read_json_with_schema normalized_output_file in
